...
interface YtsApiResponse {
    ...
    data?: { // Data might be missing on error
        movie_count: number;
        limit: number;
        page_number: number;
...
const resultsGridVariants = { /* ... (same as before) ... */ };
const resultCardVariants = { /* ... (same as before) ... */ };
    ...
    const handleSearch = async (event?: FormEvent) => {
        ...
        try {
            ...
            if (!response.ok) {
                ...
                throw new Error(`YTS API request failed: ${response.status} ${response.statusText}`);
            }
            ...
            if (err instanceof TypeError && err.message.includes('Failed to fetch')) {
                setError(`Network error or potential CORS issue fetching from YTS API. ${err.message}`);
            } else {
        ...
        >
            ...
            <S.SearchForm onSubmit={handleSearch}>
                ...
                <S.SearchButton
                    type="submit"
                    disabled={isLoading}
                    whileHover={!isLoading ? { scale: 1.03, backgroundColor: "#4aabbd" } : {}}
                    ...
                    >
                        {results.map((item) => (
                            ...
                            >
                                <S.CardContent>
                                    <S.CardHeader>
                                        {item.coverImage && <S.CoverImage src={item.coverImage} alt={`${item.title} cover`} />}
                                        <S.HeaderText>
                                    ...
                                    <S.MagnetLink
                                        ...
                                        whileHover={{ scale: 1.03, y: -2, boxShadow: "0 6px 12px rgba(97, 218, 251, 0.4)" }}
                                        whileTap={{ scale: 0.98 }}
                                        transition={{ type: "spring", stiffness: 300, damping: 15 }}
                                    >
