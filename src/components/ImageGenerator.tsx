...
const overlayVariants = { /* ... as before ... */
    hidden: { opacity: 0 },
    visible: { opacity: 1, transition: { duration: 0.3 } },
    exit: { opacity: 0, transition: { duration: 0.3 } }
};
const contentVariants = { /* ... as before ... */
    hidden: { opacity: 0, y: 20 },
    visible: { opacity: 1, y: 0, transition: { duration: 0.4, ease: "easeOut" } },
    exit: { opacity: 0, y: 20, transition: { duration: 0.3, ease: "easeIn" } }
};
const imageVariants = { /* ... as before ... */
...
const ImageGenerator: FC<ImageGeneratorProps> = ({ onClose }) => {
    ...
    useEffect(() => {
        ...
        if (!API_KEY) {
            setError('Google API Key not found. Please set REACT_APP_GOOGLE_API_KEY.');
    ...
    const handleGenerate = useCallback(async () => {
        ...
        try {
            console.log(`Sending prompt to ${IMAGE_MODEL_NAME}: "${currentPromptValue}"`);
    ...
    const handleDownload = () => {
        ...
        a.href = imageUrl;
        const filename = prompt.trim().toLowerCase().replace(/[^a-z0-9]+/g, '_').substring(0, 30) || 'generated_image';
    ...
    return (
        <S.OverlayContainer variants={overlayVariants} initial="hidden" animate="visible" exit="exit">
            ...
            >
                ...
                {!imageUrl && !isLoading && !error && API_KEY && ( // Only show placeholder if API key exists
                    <div style={{marginTop: '50px', color: '#8892b0'}}>Enter a prompt to generate an image.</div>
                )}
                {!API_KEY && ( // Show API key error prominently if missing
                    <S.ErrorMessage style={{marginTop: '50px'}}>API Key is missing. Please configure REACT_APP_GOOGLE_API_KEY.</S.ErrorMessage>
                )}
