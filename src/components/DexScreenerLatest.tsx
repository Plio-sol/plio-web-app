...
// Define variants locally as in the context provided for the styles file
const backdropVariants = {
  hidden: { opacity: 0 },
  visible: { opacity: 1, transition: { duration: 0.3 } },
  exit: { opacity: 0, transition: { duration: 0.3 } }
};
// Results grid and card variants from original context
...
const DexScreenerLatest: FC<DexScreenerLatestProps> = ({ onClose }) => {
  ...
  // --- DexScreener API Endpoint (UNCHANGED) ---
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const mappedTokens: DisplayTokenItem[] = data
            .filter((token) => token.chainId === "solana") // Keep filter logic
            .map((token) => ({
        ...
        if (
          ...
          setError(
              `Network error or potential CORS issue fetching from DexScreener API. ${err.message}`,
          );
        } else {
          setError(
  ...
  // Helper to get link class for styling/icons (UNCHANGED)
  ...
  // --- Render Logic ---
  return (
      // MODIFIED: Use OverlayContainer as the backdrop
      <S.OverlayContainer
          variants={backdropVariants} // Use backdrop variants
          initial="hidden"
          animate="visible"
          exit="exit"
      ...
      >
          ...
          {/* Close Button (MOVED INSIDE ModalWindow) */}
          {onClose && (
              <S.CloseButton
                  onClick={onClose}
          ...
          {/* DisplayTitle (MOVED INSIDE ModalWindow) */}
          <S.DisplayTitle>Newest "Dex Paid" Tokens</S.DisplayTitle>

          {/* Results Area (MOVED INSIDE ModalWindow) */}
          <S.ResultsArea>
            <AnimatePresence>
              {isLoading && (
                  <S.Spinner
            ...
            {/* Results Grid (UNCHANGED logic, just nested deeper) */}
            {!isLoading && !error && tokens.length > 0 && (
                <S.ResultsGrid
                        ...
                        <S.LinksContainer>
                          ...
                          {token.links?.map((link, index) => (
                              <S.LinkButton
                                  ...
                                  whileTap={{ scale: 0.98 }}
                              >
                                {/* Logic to display label or type (UNCHANGED) */}
                                {link.type === "twitter"
                                    ? "" // Assuming icon handles twitter
        ...
        {/* MODIFIED: End of OverlayContainer */}
      </S.OverlayContainer>
  );
};
