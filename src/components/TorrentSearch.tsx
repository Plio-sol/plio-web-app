...
const TorrentSearch: FC<TorrentSearchProps> = ({ onClose }) => {
    ...
    const handleSearch = async (event?: FormEvent) => {
        ...
        try {
            ...
            if (!dodiResponse.ok) throw new Error(`Failed to fetch DODI data: ${dodiResponse.statusText}`);
            if (!fitgirlResponse.ok) throw new Error(`Failed to fetch FitGirl data: ${fitgirlResponse.statusText}`);

            const dodiData = await dodiResponse.json();
            const fitgirlData = await fitgirlResponse.json();

            const decodedSearchTerm = decodeURIComponent(searchTerm);
            const searchTerms = decodedSearchTerm.toLowerCase().split(' ').filter(term => term);

            // --- Data Processing Function ---
            const filterAndMap = (data: any, source: 'DODI' | 'FitGirl'): SearchResultItem[] => {
                if (!data || !Array.isArray(data.downloads)) {
                    console.warn(`Invalid or missing 'downloads' array received from ${source}`);
                    return [];
                }
                ...
                data.downloads.forEach((item: RawDownloadItem, index: number) => {
                    ...
                    if (!item || typeof item.title !== 'string' || !Array.isArray(item.uris) || item.uris.length === 0) {
        ...
        >
            ...
            <S.SearchForm onSubmit={handleSearch}>
                ...
                <S.SearchButton
                    type="submit"
                    disabled={isLoading}
                    whileHover={!isLoading ? { scale: 1.03, backgroundColor: "#7a1fc2" } : {}}
            ...
            <S.ResultsArea>
                ...
                {!isLoading && !error && searched && results.length === 0 && (
                    <S.NoResultsMessage>No results found for "{searchTerm}".</S.NoResultsMessage>
                )}
                    ...
                    >
                        {results.map((item) => (
                            <S.ResultCard
                                ...
                                layout
                                whileHover={{ y: -5, borderColor: "rgba(97, 218, 251, 0.6)", boxShadow: "0 8px 25px rgba(0, 0, 0, 0.3)"}}
                            ...
                            >
                                ...
                                <S.DownloadLink
                                    ...
                                    whileHover={{ scale: 1.03, y: -2, boxShadow: "0 7px 15px rgba(138, 43, 226, 0.4)" }}
                                    whileTap={{ scale: 0.98 }}
                                    transition={{ type: "spring", stiffness: 300, damping: 15 }}
                                >
                                    Download Link
