...
const DexScreenerVolume: FC<DexScreenerVolumeProps> = ({ onClose }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      console.log(
        `Fetching from endpoint: ${activeTimeFrame} (${currentEndpoint})`,
      );
      ...
      try {
        ...
        console.error(
          `Error fetching DexScreener ${activeTimeFrame} volume data:`,
          err,
        );
        setError(
          err.message || `Could not load ${activeTimeFrame} volume data.`,
        );
      } finally {
        setIsLoading(false);
      }
